    ...
    def __encodeFloat(self, value):
        ...
        if intValue == value:
            if intValue == 0:
                ...
                self.__writeByte(value_bytes[-1])
            elif -32768 <= value <= 32767:
                self.__write('\x5e')
                self.__writeByte(256*value_bytes[-2] + value_bytes[-1])
            return
...
class Hessian2Input(object):
    ...
    def __decodeDate(self, code):
        ...
        if code == 0x4a:
            ...
            result = datetime.datetime(ts.tm_year, ts.tm_mon, ts.tm_mday, ts.tm_hour, ts.tm_min, ts.tm_sec,
                                       milliseconds * 1000)
        else:
